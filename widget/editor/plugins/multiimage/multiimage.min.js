...
(function(K){function KSWFUpload(options){this.init(options);}
...
if(data.error!==0){showError(itemDiv,K.DEBUG?data.message:self.options.errorMessage);return;}
...
self.exec('insertimage',data.url,data.title,data.width,data.height,data.border,data.align);});setTimeout(function(){self.hideDialog().focus();},0);}});});});(function(){if(window.SWFUpload){return;}
