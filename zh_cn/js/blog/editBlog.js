...
page_controller = function(p){
	...
	page_bind = function(){
		...
		editor.html(page_params.content).focus();
		...
		ybind('click',$('#add_blog'),function(){
			...
			yajax('AjaxBlog','EditBlog',{id:page_params.id,title:title,content:content},function(obj){
				if(obj.code==1){
					showTip('修改成功！');
				}
			},this);
